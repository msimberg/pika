...
namespace pika::threads::policies {
    local_queue_scheduler::local_queue_scheduler(
    ...
    std::string local_queue_scheduler::get_scheduler_name()
    {
        return "local_queue_scheduler";
    }

#ifdef PIKA_HAVE_THREAD_CREATION_AND_CLEANUP_RATES
    std::uint64_t local_queue_scheduler::get_creation_time(bool reset)
    {
        std::uint64_t time = 0;
    ...
    std::uint64_t local_queue_scheduler::get_cleanup_time(bool reset)
    {
        std::uint64_t time = 0;
    ...
    std::int64_t local_queue_scheduler::get_num_pending_misses(
    ...
    std::int64_t local_queue_scheduler::get_num_pending_accesses(
        std::size_t num_thread, bool reset)
    {
    ...
    std::int64_t local_queue_scheduler::get_num_stolen_from_pending(
        std::size_t num_thread, bool reset)
    {
    ...
    std::int64_t local_queue_scheduler::get_num_stolen_to_pending(
        std::size_t num_thread, bool reset)
    {
        std::int64_t num_stolen_threads = 0;
        if (num_thread == std::size_t(-1))
    ...
    std::int64_t local_queue_scheduler::get_num_stolen_from_staged(
        std::size_t num_thread, bool reset)
    {
        std::int64_t num_stolen_threads = 0;
        if (num_thread == std::size_t(-1))
    ...
    std::int64_t local_queue_scheduler::get_num_stolen_to_staged(
        std::size_t num_thread, bool reset)
    {
        std::int64_t num_stolen_threads = 0;
        if (num_thread == std::size_t(-1))
    ...
    void local_queue_scheduler::abort_all_suspended_threads()
    {
        for (std::size_t i = 0; i != queues_.size(); ++i)
            queues_[i]->abort_all_suspended_threads();
    }

    bool local_queue_scheduler::cleanup_terminated(bool delete_all)
    {
        bool empty = true;
        for (std::size_t i = 0; i != queues_.size(); ++i)
    ...
    bool local_queue_scheduler::get_next_thread(std::size_t num_thread,
    ...
    void local_queue_scheduler::schedule_thread(
    ...
    void local_queue_scheduler::schedule_thread_last(
        threads::detail::thread_id_ref_type thrd,
        execution::thread_schedule_hint schedulehint, bool allow_fallback,
        execution::thread_priority /* priority */)
    {
    ...
    void local_queue_scheduler::destroy_thread(
    ...
    std::int64_t local_queue_scheduler::get_queue_length(
    ...
    std::int64_t local_queue_scheduler::get_thread_count(
    ...
    bool local_queue_scheduler::is_core_idle(std::size_t num_thread) const
    {
        return queues_[num_thread]->get_queue_length() == 0;
    }

    bool local_queue_scheduler::enumerate_threads(
    ...
    std::int64_t local_queue_scheduler::get_average_thread_wait_time(
    ...
    std::int64_t local_queue_scheduler::get_average_task_wait_time(
        std::size_t num_thread) const
    {
    ...
    bool local_queue_scheduler::wait_or_add_new(std::size_t num_thread,
    ...
    void local_queue_scheduler::on_start_thread(std::size_t num_thread)
    {
    ...
    void local_queue_scheduler::on_stop_thread(std::size_t num_thread)
    {
        queues_[num_thread]->on_stop_thread(num_thread);
    }

    void local_queue_scheduler::on_error(
